...
func init() {
	...
	dir := os.Getenv("MEOWCORD_DIRECTORY")
	if dir == "" {
	...
	if dir != "" {
		Log.Debug("Using MEOWCORD_DIRECTORY")
		MeowcordDirectory = dir
	} else {
		MeowcordDirectory = path.Join(BaseDir, "meowcord.asar")
	}
}
...
func patchAppAsar(dir string, isSystemElectron bool) (err error) {
	...
	if err := WriteAppAsar(appAsar, MeowcordDirectory); err != nil {
		return err
	}
...
func (di *DiscordInstall) patch() error {
	...
	if di.isFlatpak {
		...
		Log.Debug("This is a flatpak. Trying to grant the Flatpak access to", MeowcordDirectory+"...")
		...
		args = append(args, "override", name, "--filesystem="+MeowcordDirectory)
		...
		if err != nil {
			return errors.New("Failed to grant Discord Flatpak access to " + MeowcordDirectory + ": " + err.Error())
		}
	}
	return nil
}
